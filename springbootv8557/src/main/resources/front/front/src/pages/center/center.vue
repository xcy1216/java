    ...
    <el-tabs tab-position="left" :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px 0 0 0","borderRadius":"8px","background":"rgba(255,255,255,1)"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' label="用户名" prop="shangjiahao">
            <el-input v-model="sessionForm.username" placeholder="用户编号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.password" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' label="电话" prop="dianhua">
            <el-input v-model="sessionForm.phonenumber" placeholder="电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' label="用户类型" prop="shangjiamingcheng">
            <el-input v-model="sessionForm.role" placeholder="用户类型" ></el-input>
          </el-form-item>

          <!-- <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='shanghu'" label="邮箱" prop="youxiang">
            <el-input v-model="sessionForm.youxiang" placeholder="邮箱" ></el-input>
          </el-form-item> -->
          <!-- <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='shanghu'" label="地址" prop="dizhi">
            <el-input v-model="sessionForm.dizhi" placeholder="地址" ></el-input>
          </el-form-item> -->
          <!-- <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='shanghu'" label="营业执照">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="shanghuyingyezhizhaoHandleAvatarSuccess">
              <img :style='{"border":"1px solid #9ce0b5","cursor":"pointer","minHeight":"100px","boxShadow":"inset 0px 0px 48px 0px #cef1db","color":"#3db769","borderRadius":"6px","textAlign":"center","width":"150px","fontSize":"32px","lineHeight":"100px","height":"auto"}' v-if="sessionForm.yingyezhizhao" :src="baseUrl + sessionForm.yingyezhizhao" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='shanghu'" label="手机号" prop="mobile">
            <el-input v-model="sessionForm.mobile" placeholder="手机号" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="农户号" prop="nonghuhao">
            <el-input v-model="sessionForm.nonghuhao" placeholder="农户号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="农户名" prop="nonghuming">
            <el-input v-model="sessionForm.nonghuming" placeholder="农户名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="电子邮箱" prop="dianziyouxiang">
            <el-input v-model="sessionForm.dianziyouxiang" placeholder="电子邮箱" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="地址" prop="dizhi">
            <el-input v-model="sessionForm.dizhi" placeholder="地址" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='nonghu'" label="手机号" prop="mobile">
            <el-input v-model="sessionForm.mobile" placeholder="手机号" ></el-input>
          </el-form-item> -->
...
<script>
  ...
  export default {
    ...
    created() {
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'shangjiahao', null);
      }
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'shangjiamingcheng', null);
      }
      if ('shanghu' == this.userTableName) {
      ...
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'dizhi', null);
      }
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'yingyezhizhao', null);
      }
      if ('shanghu' == this.userTableName) {
        this.$set(this.sessionForm, 'mobile', null);
      }
      if ('nonghu' == this.userTableName) {
        this.$set(this.sessionForm, 'nonghuhao', null);
      }
      if ('nonghu' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('nonghu' == this.userTableName) {
        this.$set(this.sessionForm, 'nonghuming', null);
      }
      if ('nonghu' == this.userTableName) {
      ...
      if ('nonghu' == this.userTableName) {
        this.$set(this.rules, 'nonghuhao', [{ required: true, message: '请输入农户号', trigger: 'blur' }]);
      }
      if ('nonghu' == this.userTableName) {
        this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
      }
      if ('nonghu' == this.userTableName) {
        this.$set(this.rules, 'nonghuming', [{ required: true, message: '请输入农户名', trigger: 'blur' }]);
      }
			if ('nonghu' == this.userTableName) {
    ...
    methods: {
      ...
      getSession() {
        this.$http.get('user/getInfo', {emulateJSON: true}).then(res => {
          
          if (res.body.code == 0) {
            this.sessionForm = res.body.data;
            console.log(this.sessionForm);
            localStorage.setItem('userid', res.body.data.id);
            if(res.body.data.vip) {
                localStorage.setItem('vip', res.body.data.vip);
            }
            if(res.body.data.touxiang) {
                localStorage.setItem('headportrait', res.data.data.touxiang);
            } else if(res.body.data.headportrait) {
      ...
      shanghuyingyezhizhaoHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('shanghu' == this.userTableName) {
            this.sessionForm.yingyezhizhao = 'upload/' + res.file;
          }
        }
      },
      chongzhi() {
