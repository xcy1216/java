...
<div :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
	...
	<div class="section-content">
	  <span v-for="item in infoList" :key="item.id">
        ...
        <div v-if="item.image" class="content" style="margin: 0;flex: 1;">
            <img style="max-width: 300px;max-height: 300px;border: 2px solid #EEEEEE;margin:8px 0 0 50px;" :src="$config.baseUrl+item.image">
        </div>
...
<script>
  export default {
    ...
    methods: {
      getInfo(page) {
        this.$http.get('messages/getAll', {params: {page, limit: this.pageSize,sort:"addtime", order:"desc"}}).then(res => {
          if (res.data.code == 0) {
      ...
      cpictureUploadChange(fileUrls) {
          this.form.image = fileUrls;
      },
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(this.form.image!=null) {
               this.form.image = this.form.image.replace(new RegExp(this.$config.baseUrl,"g"),"");
            }
            this.form.avatarurl = localStorage.getItem('headportrait')?localStorage.getItem('headportrait'):'';
            this.$http.post('messages/upload', this.form).then(res => {
			  console.log(res);
              if (res.body.code == 0) {
                this.$message({
                  type: 'success',
