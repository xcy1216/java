	...
	<div class="detail-preview" :style='{"width":"1200px","padding":"0 0 20px","margin":"0px auto","position":"relative","background":"none"}'>
		<div class="attr" :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px 0 0 0","overflow":"hidden","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","position":"relative","justifyContent":"space-between","height":"auto"}'>
			...
			<div class="info" :style='{"minHeight":"570px","padding":"0 20px 20px","margin":"0px","borderRadius":"8px","background":"#f9f9f9","width":"600px","height":"auto","order":"2"}'>
				...
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"left","width":"130px","fontSize":"14px","lineHeight":"24px","fontWeight":"bold","height":"40px"}'>发布日期</div>
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.date}}</div>
				</div>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get('knowledge/info/'+ this.detail.id, {}).then(res => {
			console.log(res);
            if (res.data.code == 0) {
      ...
      getStoreupStatus(){
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'knowledge', userid: localStorage.getItem('userid')}}).then(res => {
    ...
    //   getDiscussList(page) {
    //     this.$http.get('discussknowledge/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
    //       if (res.data.code == 0) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussknowledge/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                // this.getDiscussList(1);
                this.$message({
