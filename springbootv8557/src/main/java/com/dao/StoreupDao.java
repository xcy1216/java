...
public interface StoreupDao extends BaseMapper<StoreupEntity> {
	...
	@Select("SELECT * FROM storeup WHERE userid = #{userid} and tablename=#{tablename} and refid=#{refid}" )
	StoreupEntity getByUserTableRe(Long userid, String tablename, Long refid);
